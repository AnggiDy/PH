...
<body id="page-top">
  ...
  <div id="wrapper">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
		...
		<div class="card-deck">
			<div class="card text-white bg-info mb-3" style="max-width: 14rem;">
			  <div class="card-header bg-info">Jenis Udang</div>
			  <div class="card-body">
				<h5 class="card-text"><?php $query = mysqli_query($conn,'SELECT * FROM kolam where idkolam=1');
			...
			<div class="card text-white bg-info mb-3" style="max-width: 15rem;">
			  <div class="card-header bg-info">Tanggal Pembelian</div>
			  <div class="card-body">
				<h5 class="card-text"><?php $query = mysqli_query($conn,'SELECT * FROM kolam where idkolam=1');
											while($data = mysqli_fetch_array($query))
											echo $data['pembelian'];
			...
			<div class="card text-white bg-info mb-3" style="max-width: 9rem;">
			  <div class="card-header bg-info">Jumlah Bibit</div>
			  <div class="card-body">
				<h5 class="card-text"><?php $query = mysqli_query($conn,'SELECT * FROM kolam where idkolam=1');
											while($data = mysqli_fetch_array($query))
											echo $data['jumlahbibit'];
										?></h5>
			  </div>
			</div>
			
			<div class="card text-white bg-info mb-3" style="max-width: 12rem;">
			  <div class="card-header bg-info">Prediksi Panen</div>
			  <div class="card-body">
				<h5 class="card-text"><?php $query = mysqli_query($conn,'SELECT * FROM kolam where idkolam=1');
											while($data = mysqli_fetch_array($query))
											echo $data['prediksipanen'];
										?></h5>
			  </div>
			</div>
			
			<div class="card text-white bg-info mb-3" style="max-width: 13rem;">
			  <div class="card-header bg-info">Jumlah Produksi Panen</div>
			  <div class="card-body">
				<h5 class="card-text"><?php $query = mysqli_query($conn,'SELECT * FROM kolam where idkolam=1');
											while($data = mysqli_fetch_array($query))
											echo $data['jumlahpanen'];
										?></h5>
			  </div>
			</div>
		</div>
		...
		<div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Data PH Air</h6>
            </div>
            <div class="card-body">
              
				<div class="table-responsive">
					<table class="table table-hover dashboard-task-infos"id='phtable'>
						<thead>
							<tr>
								<th>#</th>
								<th>Waktu</th>
  ...
  <script src="https://cdn.jsdelivr.net/npm/chart.js@2.8.0"></script>
	...
	ctx.height=100;
		...
		function grapik(){
			$.get('PHapi_grap.php?id=1',function(data){
